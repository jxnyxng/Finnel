...
@Service
public class MarketDataSyncJobLauncher {

    private final JobLauncher jobLauncher;
    private final Job marketDataSyncJob;

    public MarketDataSyncJobLauncher(
        JobLauncher jobLauncher,
        @Qualifier(BatchJobNames.MARKET_DATA_SYNC) Job marketDataSyncJob
    ) {
        this.jobLauncher = jobLauncher;
        this.marketDataSyncJob = marketDataSyncJob;
    }

    public MarketDataSyncService.SyncResult runManualSync() {
        try {
            JobExecution execution = BatchJobLaunchSupport.runWithLockRetry(jobLauncher, marketDataSyncJob, manualParameters());
            return toSyncResult(execution);
        } catch (JobExecutionAlreadyRunningException exception) {
            return failedResult("SKIPPED_RUNNING", "Market data sync batch job is already running");
        } catch (JobRestartException | JobInstanceAlreadyCompleteException | JobParametersInvalidException exception) {
            throw new IllegalStateException("Failed to launch market data sync batch job", exception);
        }
    }

    private JobParameters manualParameters() {
        return new JobParametersBuilder()
            .addString("operation", "MARKET_DATA_SYNC")
    ...
    private MarketDataSyncService.SyncResult toSyncResult(JobExecution execution) {
        ...
        return failedResult(
            ...
            execution.getExitStatus() == null ? "Market data sync batch job ended without result" : execution.getExitStatus().getExitDescription()
        );
    }
