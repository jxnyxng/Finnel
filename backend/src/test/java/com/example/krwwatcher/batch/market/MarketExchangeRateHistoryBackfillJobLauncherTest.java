...
class MarketExchangeRateHistoryBackfillJobLauncherTest {
    ...
    void mapsBatchExecutionContextToSyncResult() throws Exception {
        ...
        context.putInt("sync.exchangeRateRows", 12);
        ...
        context.putString("sync.message", "exchangeRateHistoryBackfill=12");
        context.putString("sync.trigger", "EXCHANGE_RATE_HISTORY_BACKFILL");
        context.putString("sync.startedAt", "2026-08-30T01:00:00Z");
        context.putString("sync.nextAllowedAt", "2026-08-30T01:30:00Z");
        context.putLong("sync.remainingCooldownSeconds", 1800);
        when(jobLauncher.run(any(), any())).thenReturn(execution);

        MarketExchangeRateHistoryBackfillJobLauncher launcher = new MarketExchangeRateHistoryBackfillJobLauncher(jobLauncher, job);

        MarketDataSyncService.SyncResult result = launcher.runManualExchangeRateHistoryBackfill();

        assertThat(result.exchangeRateRows()).isEqualTo(12);
        assertThat(result.status()).isEqualTo("SUCCESS");
        assertThat(result.message()).isEqualTo("exchangeRateHistoryBackfill=12");
        assertThat(result.trigger()).isEqualTo("EXCHANGE_RATE_HISTORY_BACKFILL");
        assertThat(result.startedAt()).isEqualTo(Instant.parse("2026-08-30T01:00:00Z"));
        assertThat(result.nextAllowedAt()).isEqualTo(Instant.parse("2026-08-30T01:30:00Z"));
        ...
        assertThat(parametersCaptor.getValue().getString("operation")).isEqualTo("EXCHANGE_RATE_HISTORY_BACKFILL");
