...
export function HomePage({ onGoDashboard, onReachLastSection }: HomePageProps) {
  ...
  return (
    <section
      ...
      className="home-deck page-content-enter relative -mx-5 -my-4 h-[calc(100vh-86px)] overflow-hidden px-5 text-white"
    ...
    >
      ...
      >
      <section className="home-snap-section home-copy mx-auto grid h-[calc(100vh-86px)] max-w-6xl content-center py-14">
        <div className="-translate-y-8 max-w-5xl md:-translate-y-10">
          <div className="text-6xl leading-none md:text-7xl" aria-hidden="true">₩</div>
          <p className="mt-8 text-sm font-bold tracking-[0.24em] text-teal-100/80">KOREA WON MONITOR</p>
          <h1 className="mt-5 max-w-[980px] text-3xl font-extrabold leading-[1.22] tracking-normal sm:text-4xl md:text-6xl md:leading-[1.12]">
            환전하기 전, 원화의 위치부터.
          </h1>
          <p className="mt-7 max-w-4xl text-base font-medium leading-8 text-white/72 md:text-lg md:leading-8">
      ...
      <section className="home-snap-section home-copy mx-auto grid h-[calc(100vh-86px)] max-w-6xl content-center py-14">
        <div className="grid gap-10 md:grid-cols-[0.95fr_1.05fr] md:items-center">
          <div>
            <h2 className="max-w-[760px] text-3xl font-extrabold leading-[1.22] tracking-normal md:text-5xl md:leading-[1.14]">
              환율은 매번 찾아보기 어렵습니다.
            </h2>
          ...
          <div className="grid gap-6">
            {servicePoints.map((point) => (
              <section className="grid grid-cols-[3.5rem_1fr] items-start gap-5" key={point.title}>
                <span className="text-4xl leading-none md:text-5xl" aria-hidden="true">{point.emoji}</span>
                <span>
                  <h3 className="text-lg font-extrabold tracking-normal text-white md:text-xl">{point.title}</h3>
                  <p className="mt-2 max-w-xl text-sm font-medium leading-7 text-white/64 md:text-base">{point.body}</p>
                </span>
              </section>
            ))}
          </div>
      ...
      <section className="home-snap-section home-copy mx-auto grid h-[calc(100vh-86px)] max-w-6xl content-center py-14">
        <div className="max-w-5xl">
          <h2 className="max-w-[840px] text-3xl font-extrabold leading-[1.22] tracking-normal md:text-5xl md:leading-[1.14]">
            필요한 정보만 한 흐름으로 봅니다.
          </h2>
          <p className="mt-6 max-w-4xl text-base font-medium leading-8 text-white/68 md:text-lg">
        ...
        <div className="mt-12 grid gap-7 sm:grid-cols-2 lg:grid-cols-3">
          {tabHints.map((hint) => (
            <section className="grid gap-3" key={hint.title}>
              <span className="text-5xl leading-none" aria-hidden="true">{hint.emoji}</span>
              <h3 className="text-lg font-extrabold tracking-normal text-teal-100 md:text-xl">{hint.title}</h3>
              <p className="max-w-lg text-sm font-medium leading-7 text-white/64">{hint.body}</p>
            </section>
          ))}
        </div>
      </section>

      <section className="home-snap-section home-copy mx-auto grid h-[calc(100vh-86px)] max-w-6xl content-center justify-items-center py-14 text-center">
        <div className="grid max-w-5xl justify-items-center">
          ...
          <button
            className="mt-8 inline-flex h-11 items-center justify-center rounded-full border border-teal-200/30 bg-teal-400/18 px-5 text-sm font-extrabold text-white shadow-lg shadow-teal-950/20 backdrop-blur-md transition-colors duration-150 hover:bg-teal-300/28"
      ...
      <div className="pointer-events-none absolute bottom-7 left-1/2 flex -translate-x-1/2 items-center gap-2" aria-hidden="true">
