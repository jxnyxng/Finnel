    ...
    <section className="grid min-w-0 gap-3">
      <header className="page-tab-header">
        <div className="min-w-0">
          ...
          <p className="page-tab-description">스크롤 없이 오늘의 환율, 달러, 지표, 소식 흐름을 빠르게 확인합니다.</p>
        </div>
      ...
      <section className="grid min-w-0 gap-3 xl:min-h-[31rem] xl:grid-cols-[19rem_minmax(24rem,1.16fr)_minmax(0,0.84fr)]">
        <section className="grid min-w-0 gap-3">
          ...
          <section className="glass-card rounded-2xl p-3.5">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase text-teal-700">Indicators</p>
        ...
        <section className="grid min-w-0 gap-3 xl:grid-rows-[auto_minmax(0,1fr)]">
          ...
          />
          <section className="glass-card rounded-2xl p-3.5 xl:min-h-0">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase text-teal-700">News & Policy</p>
                <h3 className="mt-1 text-base font-bold text-zinc-950">뉴스·정책</h3>
              </div>
              <span className="rounded bg-zinc-100 px-2 py-1 text-[11px] font-bold text-zinc-600">{combinedFeedCount}건</span>
            </div>
            <div className="mt-3 grid gap-2 xl:max-h-[22rem] xl:overflow-y-auto xl:pr-1">
        ...
        <section className="grid min-w-0 gap-3 xl:grid-rows-[auto_minmax(0,1fr)]">
          <section className="glass-card grid min-w-0 gap-3 rounded-2xl p-4">
            <div className="min-w-0">
              <p className="text-xs font-bold uppercase text-teal-700">3M Flow</p>
              <h3 className="mt-1 text-base font-extrabold leading-6 text-zinc-950">최근 3개월 환율 흐름</h3>
              <p className="mt-2 text-sm font-semibold leading-6 text-zinc-600">{getThreeMonthFlowText(usdKrwThreeMonthPosition)}</p>
            </div>
            <div className="grid justify-items-center rounded-xl border border-zinc-200 bg-zinc-50 p-3">
              <DonutGauge caption="3개월" compact score={threeMonthScore} title={getPositionShortLabel(usdKrwThreeMonthPosition)} />
            </div>
          </section>

          <section className="glass-card grid min-h-0 rounded-2xl p-3.5">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase text-teal-700">Briefing</p>
                <h3 className="mt-1 text-base font-bold text-zinc-950">브리핑 메시지</h3>
              </div>
            </div>
            <div className="scrollbar-none min-h-0 overflow-y-auto rounded-xl border border-zinc-200 bg-zinc-50 p-2">
              {brief.slice(0, 3).map((line) => (
                <p className="rounded-lg bg-white px-3 py-2 text-xs font-medium leading-5 text-zinc-600" key={line}>{line}</p>
              ))}
            </div>
          </section>
        </section>
      ...
      {dashboardLoadState === 'error' && !dashboard ? (
        <section className="glass-card rounded-2xl p-5 text-sm font-medium text-zinc-700">{dashboardEmptyText}</section>
      ) : null}
...
function MetricTile({ helper, label, tone, value }: { helper: string; label: string; tone: Direction; value: string }) {
  return (
    <article className="glass-card min-w-0 rounded-2xl p-4">
      <div className="flex items-center justify-between gap-2">
        <p className="truncate text-xs font-bold uppercase text-zinc-500">{label}</p>
        <span className={`rounded px-2 py-0.5 text-[11px] font-bold ${getDirectionBadgeClass(tone)}`}>{getDirectionLabel(tone)}</span>
      </div>
      <p className="mt-3 truncate text-xl font-extrabold text-zinc-950">{value}</p>
      <p className="mt-2 text-clamp-1 text-xs font-semibold leading-5 text-zinc-500">{helper}</p>
    </article>
  );
}
...
function IndicatorPill({ indicator }: { indicator: DomesticIndicator }) {
  ...
  return (
    <article className="min-w-0 rounded-xl border border-zinc-200 bg-white px-3 py-3">
...
function MiniSparkline({ label, series }: { label: string; series: TimeSeriesPoint[] }) {
  ...
  return (
    <div className="glass-card overflow-hidden rounded-2xl p-3.5">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-xs font-bold uppercase text-teal-700">{label}</p>
          <p className="mt-1 text-lg font-extrabold text-zinc-950">{latestValue === null ? '-' : `${formatValue(latestValue, 2)}원`}</p>
        </div>
        <span className={`rounded px-2 py-1 text-[11px] font-bold ${getNumberToneClass(change)}`}>
          {change === null ? '-' : `${change >= 0 ? '+' : ''}${formatValue(change, 2)}%`}
        </span>
      </div>
      <div className="mt-3 h-32 rounded-xl bg-zinc-50 px-1 py-2">
        {chartData.length > 1 ? (
          <ResponsiveContainer height="100%" width="100%">
            <LineChart data={chartData} margin={{ top: 8, right: 10, bottom: 0, left: 0 }}>
              ...
              <Tooltip
                ...
                cursor={{ stroke: '#cbd5e1', strokeDasharray: '3 4', strokeWidth: 1 }}
                isAnimationActive={false}
              />
              <Line
                activeDot={{ fill: '#ffffff', r: 4, stroke: '#18a999', strokeWidth: 2 }}
                dataKey="value"
                dot={false}
                isAnimationActive={false}
                stroke="#18a999"
          ...
          <div className="grid h-full place-items-center text-xs font-bold text-zinc-400">데이터 대기</div>
        )}
      </div>
    </div>
  );
...
function FeedRow({ compact = false, item }: { compact?: boolean; item: { href: string | null; kind: string; meta: string; title: string } }) {
  const body = (
    <div className={`grid min-w-0 grid-cols-[auto_minmax(0,1fr)] items-start gap-2 rounded-xl border border-zinc-200 bg-white px-3 hover:border-teal-200 hover:bg-teal-50 ${compact ? 'py-2' : 'py-2.5'}`}>
