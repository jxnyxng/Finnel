...
| 미국 금융여건·리스크 | FRED | 10년 국채금리 `DGS10`, VIX `VIXCLS`, WTI `DCOILWTICO` | 전체 시장 데이터 수집 시 최신 발표값 저장 |
...
| 추가 연동 후보 | 열린재정/공공데이터포털/KOSIS/거래소 계열 | 재정수지·국가채무, 외국인 주식·채권 자금, CDS, 교역조건 | 별도 인증키 또는 활용신청 후 연동 |
